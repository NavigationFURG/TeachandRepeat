...
- Stores path data in a structured text format for later use
- Manages named dock locations (special waypoints within the path)
- Provides real-time visualization in RViz through marker publications
...
- **'odom' frame**: The node subscribes to the ``/odom`` topic (Odometry message). This provides relative positioning based on wheel odometry or other proprioceptive sensors. The odometry frame is local to the robot's starting position and accumulates drift over time. Recording in the odom frame is faster and more responsive, making it suitable for real-time path recording.

- **'map' frame**: The node subscribes to the ``/amcl_pose`` topic (PoseWithCovarianceStamped message). This provides globally localized position through the Adaptive Monte Carlo Localization algorithm. The map frame is global and consistent with a pre-built map, eliminating drift but requiring a map to be loaded. Recording in the map frame produces globally consistent paths.

The choice between odom and map depends on the application context:
- Use **'odom'** for immediate feedback and local path recording without map dependency
- Use **'map'** for long-distance paths or when global consistency is required
...
- **False** (default): Only (x, y) coordinates are recorded. Each waypoint contains 2D Cartesian position information. The repeat system will then compute optimal orientations during path following, or the user can specify desired orientations separately.

- **True**: Both position (x, y) and orientation (yaw) are recorded. Each waypoint contains 3D position-orientation information. The repeat system will attempt to follow both the spatial path and the recorded headings, useful for tasks where the robot's direction is critical (e.g., docking, manipulation, or paths that require specific approach angles).
...
4. **Active Recording**: For each odometry/localization update received while ``recording`` is True:
   ...
   - A 1.5-second sleep is introduced to prevent over-sampling of data points
...
- **Manual Guidance Alignment**: Aligns with typical human reaction times when manually controlling the robot

The resulting sampling rate of approximately one waypoint per 1.5 seconds means a 100-meter path with 10 meters between waypoints would require approximately 150 seconds (2.5 minutes) to record.
...
Note: The z-coordinate of orientation is used to represent the yaw angle (rotation around the vertical axis). This is a 2D navigation convention where yaw is the heading angle in radians, typically in the range [-π, π].
...
Each line in the text file contains two comma-separated floating-point values representing the x and y coordinates of a single waypoint:
...
- Each value represents meters (for typical ROS navigation stacks)
...
- Third value: yaw angle in radians, typically in range [-π, π] (approximately [-3.14159, 3.14159])
- No header or metadata
...
- **message** (string): Descriptive message including file path or error details
7. **Monitor RViz** to verify the red line follows the intended path and docks appear as blue spheres
8. **Stop the robot** when the path is complete
9. **Call SavePath service** to persistently store the recorded path
10. **Verify the file** was created in ``path_saves/`` directory with correct format

The recorded path can then be loaded and executed by the Repeat module for autonomous path following.Monitor RViz** to verify the red line follows the intended path
7. **Stop the robot** when the path is complete
8. **Call SavePath service** to persistently store the recorded path
9