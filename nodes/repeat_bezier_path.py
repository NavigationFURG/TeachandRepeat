...
from geometry_msgs.msg import Point, Twist, TwistStamped, PoseWithCovarianceStamped
...
from read_coords_from_file import read_points_from_file as getPointsFromFile
...
class RepeatBezierPath(Node):
    ...
    def __init__(self):
        ...
        # Marker configuration that shows raw points
        self.raw_points_marker = Marker()
        self.raw_points_marker.header.frame_id = frame_id
        self.raw_points_marker.type = Marker.POINTS
        self.raw_points_marker.action = Marker.ADD
        self.raw_points_marker.pose.orientation.w = 1.0
        self.raw_points_marker.scale.x = 0.07
        self.raw_points_marker.scale.y = 0.07
        self.raw_points_marker.color.r = 0.0
        self.raw_points_marker.color.g = 1.0
        self.raw_points_marker.color.b = 0.0
        self.raw_points_marker.color.a = 1.0
        ...
        # enviados.
        ctrl_pts = BezierFitDemo(self.teleop_path_points)
        ctrl_pts = ctrl_pts.T
    ...
    def callback_scan(self, msg : LaserScan):
        '''
        This function receives the laser scan data from the robot.
        '''
        self.obstacle_stop(msg.ranges)

    def callback_odometry(self, msg : PoseWithCovarianceStamped):
        '''
        This function receives the odometry data from the robot.
        '''
    ...
    def update(self):
        '''
        This function updates the robot position and the look ahead paths.
        '''
        if self.use_sim == False:
            msg = Twist()
        else:
        ...
        # For each steering angle update the points from the robot 
        # position.
        for steer_angle, points in self.generated_lookahead.items():
            ...
            # TODO: Change distance calculation to use numpy
            d = np.sqrt(points[:, 0]**2 + points[:, 1]**2)

            # Calculate the angle of each point
            angle = np.arctan2(points[:, 1], points[:, 0])

            # The points was generated in the local frame of the robot
            # so we need to rotate the points to the global frame
        ...
        # Update the look ahead paths marker
        ...
        for p in self.teleop_path_points:
            point = Point()
            point.x, point.y = p[0], p[1]
            self.raw_points_marker.points.append(point)
        self.raw_points_marker_pub.publish(self.raw_points_marker)

        # Update the reference points of the bezier curve
        self.bezier_few_points = self.bezier_path_coords[self.new_min:self.new_max]

        # Verify the best local path
        for angle, lookahead_path_points in self.lookahead_updated.items():
            ...
            # Select the best local path
            if min_cost is None or cost < min_cost:
                min_cost = cost
                self.desired_steering_angle = angle

                # Assing the best local path to the variable
        ...
        if self.use_sim == False:
            ...
            msg.twist.linear.x = (left_wheel_speed + right_wheel_speed) / 2.0  # Velocidade linear
            msg.twist.angular.z = (right_wheel_speed - left_wheel_speed) / self.distance_btw_wheels  # Velocidade angular
        ...
        self.raw_points_marker.points = []


    def generate_lookahead(self):
        '''
        The function generates the look ahead paths for each steering angle.
        It's similar to the DWA local planner window or Stanley controller.
        '''
